...
const answercheck = () => {
  let answer = Prompt(
    "Do you know your sign? Please enter Y or N: "
  ).toUpperCase();
...
const verify_sign = (sign) => {
    ...
    if (!Object.keys(signObject).includes(sign)) {
      do {
        ...
        console.log("You don't know your sign? No problem! I'll help you find it!");
        let birthday = Prompt("Please enter your birthday in the format MM/DD/YYYY: ");
        return Calculate_sign(birthday);
    }
};
...
const main = () => {
    ...
    const find_sign = birth_to_sign(Calculate_sign());
    
    //const Fortune_teller = Horoscope(sign);
    
    if (first_sign) {
        return Horoscope(first_sign);
        
    } else {
        return Horoscope(find_sign);
    }
}
main();